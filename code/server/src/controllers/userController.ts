...
class UserController {
    ...
    // TO TEST EXTENSIVELY WITH JEST, else reimplement getUserByUsername() in userDAO.ts, warning getUserByUsername() is used in other UserDAO.ts methods!!! do not change it, but rather write another method if necessary
    // async getUserByUsername(user: User, username: string) :Promise<User> {
    //     try {
    ...
    async updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string) : Promise<User> {
        return this.dao.updateUserInfoByUsername(user, name, surname, address, birthdate, username);
     }
}
