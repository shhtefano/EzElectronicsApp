...
class ProductController {
    ...
    async registerProducts(model: string, category: string, quantity: number, details: string | null, sellingPrice: number, arrivalDate: string | null) :Promise<void> { 
        return this.dao.registerFirstArrival(model, category, quantity, details, sellingPrice, arrivalDate);
    }
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null) : Promise<number> { 
        return this.dao.registerQuantityUpdate(model, newQuantity, changeDate);
     }
     ...
     * @returns A Promise that resolves to the new available quantity of the product.
     */
    async sellProduct(model: string, quantity: number, sellingDate: string | null) : Promise<void> {
        return this.dao.sellProducts(model, quantity, sellingDate);
    }
     ...
     * @returns A Promise that resolves to `true` if all products have been successfully deleted. -> in API.md reponse body content is none -> change to Promise<void>
     */
     ...
     * @returns A Promise that resolves to `true` if the product has been successfully deleted -> in API.md reponse body content is none -> change to Promise<void>.
     */
