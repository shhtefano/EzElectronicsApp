...
class ProductRoutes {
    ...
    initRoutes() {
         ...
         * It requires the user to be logged in and to be an admin or a manager.
        ...
        this.router.post(
            ...
            body("arrivalDate").optional({ checkFalsy: true }).isString().isDate({format: "YYYY-MM-DD"}),
        ...
        this.router.patch(
            ...
            body("quantity").isInt({ min: 1 }),
            body("changeDate").optional({ checkFalsy: true }).isString().isDate({format: "YYYY-MM-DD"}),
            this.errorHandler.validateRequest,
        ...
        this.router.patch(
            ...
            param("model").isString().notEmpty(),
            body("quantity").isInt({ min: 1 }),
            body("sellingDate").optional({ checkFalsy: true }).isString().isDate({format: "YYYY-MM-DD"}),
            this.errorHandler.validateRequest,
        ...
        this.router.get(
            ...
            this.authenticator.isLoggedIn,
            this.authenticator.isAdminOrManager,
            // optional -> if present, its value os 1 among the listed ones; else absent
            query("grouping").optional({ checkFalsy: true }).isString().isIn(["category", "model"]),
            query("category").optional({ checkFalsy: true }).isString().isIn([Category.SMARTPHONE,Category.LAPTOP, Category.APPLIANCE]),
         ...
         * !!! NOTE: and to be a customer. --> NOT IN THE API.md document, ignore it
         * It can have the following optional query parameters:
         * - grouping: string. It can be either "category" or "model". If absent, then all products are returned and the other query parameters must also be absent.
         * - category: string. It can only be present if grouping is equal to "category" (in which case it must be present) and, when present, it must be one of "Smartphone", "Laptop", "Appliance".
         * - model: string. It can only be present if grouping is equal to "model" (in which case it must be present and not empty).
